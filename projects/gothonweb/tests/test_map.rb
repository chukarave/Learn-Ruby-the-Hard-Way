...
class TestMap < Test::Unit::TestCase
  ...
  def test_gothon_game_map()
      ...
      assert_equal(Map::LASER_WEAPON_ARMORY.go('*'), Map::GENERIC_DEATH)

      room = Map::LASER_WEAPON_ARMORY.go('0132')
      assert_equal(room, Map::THE_BRIDGE)

      assert_equal(Map::THE_BRIDGE.go('throw the bomb'), Map::GENERIC_DEATH)


      room = Map::THE_BRIDGE.go('slowly place the bomb')
      assert_equal(room, Map::ESCAPE_POD)

      assert_equal(Map::ESCAPE_POD.go('*'), Map::THE_END_LOSER)

      room = Map::ESCAPE_POD.go('2')
      assert_equal(room, Map::THE_END_WINNER)
  end
